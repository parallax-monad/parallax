...
Status: DELIVERY GATE FOR DETERMINISTIC CHECK APPLICATION / RUN LIFECYCLE PATHS — LIVE SIMULATION SUCCEEDED ON THE TEMPORARY MOSS PIN; EVIDENCE PENDING NODE v22.23.2 REGENERATION
...
(`simulatorPinnedBlock=93985841`), and the 24-gate acceptance passed
(`liveSuccess=true`, `P0_LIVE_READY`). The historical "FlipOrderUpdated
unsupported" blocker is closed **for this exact pin only**.

Merge caveat: the committed fixture was generated under Node v24.14.0 while the
runtime contract pins **Node v22.23.2**. It is
**SUPERSEDED_PENDING_NODE22_REGEN** and must be regenerated by a real live
smoke under Node v22.23.2 before it counts as final merge evidence. Failures and
configuration artifacts under `.smoke-live/` remain diagnostic, not delivery
success evidence.
