...
| `intent.minimumReceived` | normalized intent | decimal string | Optional user slippage floor | optional | caller | recorded only when supplied | `INFERRED_FROM_CODE` — the optional field is supported by current code/contracts (`generic-evidence.ts`, `intent.ts`) and can be populated from the economic boundary, but there is **no non-null canonical real observation** of a supplied value in the qualified success fixture, so a real runtime observation for a supplied value is `UNQUALIFIED` / not independently runtime-qualified (absence from one real fixture is **not** `MOCK_ONLY`) | absent → omitted | `intent.minimumReceived` | no | Contract Owner |
...
| Field | Raw/source path | Raw type | Meaning / units | Req | Source | Validity | Class | Missing/malformed | Provisional mapping | Opaque? | Owner |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `action` (payload) | `raw.action.children[].transaction` (`from`/`to`/`data`/`value`) | object array | Exact **unsigned** transaction material | required for simulation | moss | byte-identical to `simulation.results[].transaction` in the real fixture | `REAL_OBSERVED` | empty children → `action.value=null`; if an explicit `NO_ROUTE` terminal outcome has already been classified (from an explicit `QUOTE`/`ACTION` route error) it is preserved, because `executionStatus()` checks the `NO_ROUTE` error before the incomplete-coverage fallback; otherwise incomplete/missing action material resolves conservatively to `executionStatus=UNKNOWN` | **Opaque.** Only the summary is projected into generic `action` | **yes** | Backend Owner (target prepared-execution binding) |
...
**Empty action vs `NO_ROUTE` precedence.** An explicit `QUOTE`/`ACTION` `NO_ROUTE` classification is a valid terminal outcome and is recognized before the incomplete-simulation/action fallback: `executionStatus()` returns `NO_ROUTE` from the normalized error **before** it tests simulation coverage, so an empty or missing action must not overwrite an already classified `NO_ROUTE`. Only when no `NO_ROUTE` has been classified does missing action material resolve conservatively toward `executionStatus=UNKNOWN` / incomplete evidence. No production normalization change is implied by this note; it documents existing behavior.
...
| `results[].revertReason` | `raw.simulation.results[i].revertReason` | string | Provider revert string | optional | moss | first string found | `REAL_OBSERVED` — a historical real observation exists in `fixtures/chain-evidence/kuru/usdc-to-mon/` (`revertReason.value="execution reverted"`, `source="moss"`, `reproducibility="REPRODUCIBLE"`), with the caveat that it is from the older `d09b38cb…` baseline and its run halted with `simulationCoverage.complete=false` (`haltReason: "execution reverted"`), so it is **not** sufficient for current pinned-runtime capability qualification; the canonical success fixture has `revertReason=null` | absent → `revertReason.value=null` + `limitation` | Currently `providerData.revertReason`; first-class generic field = Contract Owner decision | yes (until decided) | Contract Owner |
...
| `isReplay` | per-field on each Moss `Sourced<T>` field; set `true` by `replayKuruEvidence` for every sourced field; live normalization does not set it per field | `boolean` / absent | Moss normalization/internal field-level replay marker | optional | derived | present only on replayed evidence inside `@parallax/moss-bridge` | `RULE_DERIVED` (Moss-internal) | absent → field is not individually marked as replayed | **Not retained as a generic per-field `EvidenceField` property today:** `toEvidenceField(...)` does not copy `isReplay` across the generic boundary. Generic replay truthfulness is carried by top-level `provenance.mode = RECORDED_REPLAY` (via `evidenceMode(...)`); no new generic field is introduced by BE-033 | yes (Moss-internal until Contract Owner decides otherwise) | Contract Owner |
...
- `isReplay` stays a Moss-internal field-level signal. At the generic boundary, generic `EvidenceField` has no `isReplay` property and `toEvidenceField(...)` does not copy one; replay truthfulness crosses the boundary only as top-level `provenance.mode = RECORDED_REPLAY`. Generic Evidence types are **not** modified by BE-033.
...
- `MOCK_ONLY` stays reserved for synthetic mock / `RULE_TEST_INPUT` fixtures (for example `reverted`, `integration-error`, `missing-evidence`, `no-route`). Concretely: the synthetic `fixtures/chain-evidence/kuru/reverted/` rule input stays `MOCK_ONLY`, while the historical `usdc-to-mon` `revertReason` is `REAL_OBSERVED` with the incomplete-baseline caveat above — the two must not be conflated.
...
- This does not decide the generic final `revertReason` contract; whether `revertReason` becomes a first-class generic field remains a Contract Owner decision (§12).
