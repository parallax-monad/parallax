...
| E | Real revert/failure/timeout samples | `UNAVAILABLE_REAL_EVIDENCE` for complete/current coverage; historical `usdc-to-mon` carries a real but incomplete revert signal | §8; **no new live probe run** |
...
**No item is classified `MOCK_OR_RULE_ONLY` as a satisfying proof.** Where only mock/rule evidence exists (revert normalization, integration failure classification), the item is resolved as `UNAVAILABLE_REAL_EVIDENCE` (or, for revert, as a historical but incomplete real signal that does not qualify the capability) plus a documented fail-closed expectation, exactly as #58 permits.
...
**`simulatorPinnedBlock` is OUTPUT, not an input prerequisite.** The runner executes Provider evaluation first and returns `simulatorPinnedBlock` as Provider evaluation OUTPUT provenance; `assertLiveProvenance(...)` then validates it for simulated non-`NO_ROUTE` results and fails closed *after* evaluation when it is missing or invalid. A missing simulator pin is therefore **not** an unsupported input shape, and it is deliberately **not** a field of `MossPreparedExecutionInput` V1 (§7.2), which carries only the Backend preparation `blockContext`. The four block layers stay distinct: Backend preparation block (INPUT); Moss stage block, Moss quote/action evidence block, and `simulatorPinnedBlock` (all OUTPUT).
...
| Execution result | `VERIFIED_RUNTIME` | Canonical real fixture `executionStatus=SUCCESS`. A historical real revert signal does exist (`usdc-to-mon` `revertReason="execution reverted"`), but that run halted with incomplete coverage, so it did not produce a complete execution-qualified `REVERTED` outcome and normalized execution there stays `UNKNOWN`; `NO_ROUTE` has no real Moss sample (§8). |
...
| Revert reason | `UNKNOWN` (current/runtime-qualified coverage); `SUPPORTED_DOC_ONLY` as code behavior | A historical `REAL_OBSERVED` revertReason exists in `fixtures/chain-evidence/kuru/usdc-to-mon/`: `revertReason.value="execution reverted"` with `source="moss"` and `reproducibility="REPRODUCIBLE"` on the older `d09b38cb…` baseline. That run halted with `simulationCoverage.complete=false` (`haltReason: "execution reverted"`), so it does **not** qualify complete/current pinned-runtime revert behavior and the capability state remains `UNKNOWN`. `packages/moss-bridge/src/errors.ts` defines revert classification, and the synthetic `fixtures/chain-evidence/kuru/reverted/` is `RULE_TEST_INPUT`/`real=false`. |
...
| Timeout (stage) | `INFERRED_FROM_CODE` (`StageTimeoutError`, `Promise.race` 30s) | No | Moss `EvidenceProviderError` thrown with `TIMEOUT`; **no `GenericEvidence` is produced, so there is no Generic `provider.status` value (`providerStatus: null` at that boundary)**; Backend `timeout` | Yes | stage name, elapsed ms | — | Timeout is never mapped to STOP or a verdict |
| Timeout (overall) | `INFERRED_FROM_CODE` (90s default) | No | Same thrown-before-`GenericEvidence` boundary | Yes | overall deadline | — | Same |
...
| Revert / failed evaluation | `MOCK_ONLY` (`fixtures/chain-evidence/kuru/reverted/`, `real=false`) + `RULE_DERIVED` derivation from `simulation.results[].reverted`; a historical `REAL_OBSERVED` incomplete revert signal also exists in `usdc-to-mon` (`executionStatus` stays `UNKNOWN`) | **No (complete/current)** | Moss `executionStatus=REVERTED` with `integrationStatus=OK`; generic `provider.status=SUCCESS` + `execution.status=REVERTED`; Risk remains `UNKNOWN` | No | `revertReason` when present | — | A verified revert is *not* a Provider failure; it must not become `PROCEED` |
...
| `moss-reverted-mock` | `fixtures/chain-evidence/kuru/reverted/` | `false` | `RULE_TEST_INPUT` | Revert normalization/rule behavior only (`source: "mock"`) | Any real Provider revert response of its own (the separate historical `usdc-to-mon` recording is the real revert signal, and it halts incomplete) |
