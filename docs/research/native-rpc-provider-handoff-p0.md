...
Status: **PARTIALLY_QUALIFIED** for the Arbitrum Sepolia × Camelot V3 scenario as of 2026-09-16. Provider code is real-capable; a canonical Product target and executable swap path are not yet established. This handoff is rebuilt on merged PR #68 at main `b09bc771a2517aa0d67006c9b0914b36fc7ca21f`. It does not decide final GenericEvidence, Risk, Product, or Receipt semantics.

## Integration boundary

`providerId` is `native-rpc-arbitrum`. Construct through `createNativeRpcProvider({ rpcUrl, mode: "LIVE" })`, or inject a `NativeRpcClient` for deterministic tests, then register its factory-created adapter. `mode` is **required** whenever a `client` or `rpcUrl` is supplied: an endpoint-backed provider may not inherit a default truthfulness mode, because a silent `MOCK` fallback would label real RPC observations as `source: mock` / `NOT_REPRODUCIBLE` and falsify provenance. Construction without an explicit `mode` throws. Only a provider with no runtime seam at all keeps the default. The endpoint is explicit: Backend may pass its validated `ARBITRUM_RPC_URL` into `nativeRpc.rpcUrl`; the Provider does not read environment variables or choose a fallback. `ARBITRUM_SEPOLIA_RPC_URL` is only the existing research probe override. Never log either value. No wallet, signer, or broadcast method exists.
...
`createNativeRpcProvider` is deliberately **not** re-exported from the `@parallax/api` package entry (`apps/api/src/index.ts`). The supported Backend import is the internal module `apps/api/src/backend/native-rpc-provider.ts` — an in-repo consumer at `apps/api/src` uses the relative specifier `./backend/native-rpc-provider.js`, exactly as `apps/api/src/backend/native-rpc-runtime.test.ts` uses `./native-rpc-provider.js`. The current Backend convention already accepts Provider adapters as explicit dependencies: `createArbitrumProductionComposition({ providers })` takes the factory-created adapter without importing the concrete Provider. Promoting `createNativeRpcProvider` to the package entry is therefore a separate public-surface decision, not a provider integration requirement.

Before the pinned-block check, the Provider calls `eth_chainId`, validates its hex quantity, and requires Arbitrum Sepolia (`0x66eee`). A malformed, unavailable, or mismatched endpoint identity stops before block or transaction RPC. `intent.chainId` and `preparedExecution.chainId` describe the requested/evaluated chain; the Provider emits `nativeRpc.chainId` from the actual RPC response. The current Backend-owned `native-rpc-evidence.ts` mapper does not consume this candidate or populate `provenance.observedChainId`; that remains a Backend follow-up under #65/#53. The requested chain is never substituted as observation proof.
...
The Provider's `success` means the two RPC methods returned valid values at the verified block. It is **partial evidence**: no receipt, full asset-change set, state diff, outcome, or economic-constraint verdict is established. The current Backend-owned GenericEvidence mapping keeps `provider.status = UNKNOWN` and those scopes unknown. Backend owns Registry, pipeline, system composition, and this projection; this PR does not change that mapper. Contract Owner decides its final semantics.

## Capability matrix

| Surface | Scope and input | Output and units | Provenance and current evidence | Limit / failure |
| --- | --- | --- | --- | --- |
| Chain identity | `eth_chainId` before block/transaction calls | Hex RPC quantity; observed chain ID is 421614 only on match | Provider candidate and sanitized response snapshot; live chain observation in the existing qualification fixture | Malformed, unavailable, or wrong chain fails closed without block or transaction calls |
...
This Provider-owned delta adds the concrete client, Provider, tests, handoff, and read-only qualification fixture on current main. The fixture records observations made on 2026-09-16; its `repositoryBaseCommit` is historical provenance, not the rebuilt PR base. PR #63 real swap qualification remains external. No signing or broadcasting is implemented.
