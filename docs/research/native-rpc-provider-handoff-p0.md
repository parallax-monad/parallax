...
Before the pinned-block check, the Provider calls `eth_chainId`, validates its hex quantity, and requires Arbitrum Sepolia (`0x66eee`). A malformed, unavailable, or mismatched endpoint identity stops before block or transaction RPC. `intent.chainId` and `preparedExecution.chainId` describe the requested/evaluated chain; `provenance.observedChainId` comes only from the observed `nativeRpc.chainId` Provider candidate. The requested chain is never substituted as observation proof. Chain identity appears in `checkedScope` only after the RPC observation matches.
