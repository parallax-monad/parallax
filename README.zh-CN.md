...
Parallax 通过将异构 Evidence 转换为范围明确的诊断、量化的候选操作和与状态绑定的再次验证，来补上这段决策缺口。
...
- 将有证据支持的 Relevant Action 与本次结果不支持的修改分开；
...
目标分解是 `Chain × Protocol × Evidence Provider`。它将 Provider-specific 类型留在 Core 之外，并把 Evidence 获取与 Product/Risk 决策语义分开。

## 产品原则

- 不猜测用户意图；调用方提供目标和阈值时才使用明确约束；
- 诊断实际差距，并用易懂的语言解释 Cause；
- 展示可控变量，并量化候选修改；
- 用户未明确表达意图时，展示多个经过验证的反事实选项，而不是静默替用户选择；
- 使用新的报价、已准备的未签名交易、模拟和结果 Evidence 验证建议；
- 验证与状态绑定：链上状态变化后必须重新检查；
- 最终决定权保留在用户手中。

新手体验不要求提供明确阈值：Parallax 可以展示多个经过验证的反事实选项。高级 DeFi 用户、开发者、SDK 和 Agent 可以提供价格影响、有效汇率、Gas、总成本或目标输出等约束，并使用同一套“诊断 → 量化修正 → 再次验证”模型。
...
当 Evidence 支持时，可以展示经过明确评估的替代路径。全市场优化不属于 P0 范围。
...
fixtures/              记录的 Evidence 与 Replay 样例
scripts/               验证与冒烟工具
```
...
| `POST /api/check` | 对标准化 Swap Intent 执行签名前检查；再次检查通过 `parentRunId` 关联，并且只允许修改一个 Intent 条件。 |
| `GET /api/runs/:runId` | 按 ID 返回一个已持久化的 Check Run。 |
| `GET /api/replay/:id` | 返回冻结的记录回放样例，不会被替换为实时 Check。 |
...
Parallax 是用于解释和验证范围受限的签名前决策的实验性软件。Evidence 可能不完整或不可用；用户必须自行核验交易细节。`UNKNOWN` 不是通过，`PROCEED` 只在已检查范围内成立，不构成安全保证。
