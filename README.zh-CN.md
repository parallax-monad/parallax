...
新手体验不要求提供明确阈值：Parallax 可以展示多个反事实选项，并明确区分已经重新验证的交易调整与仍需在条件满足后重新检查的条件性建议。高级 DeFi 用户、开发者、SDK 和 Agent 可以提供价格影响、有效汇率、Gas、总成本或目标输出等约束，并使用同一套“诊断 → 量化修正 → 再次验证”模型。
